  ...
  return (
    <div>
      <div
        ...
        className="bg-green-400/30 flex py-3.5 px-4  border border-gray-300 text-gray-900 text-sm outline-none rounded-lg h-12  focus:ring-green-500 focus:border-green-500 w-[99%] p-2.5 dark:bg-green-400/30 dark:border-green-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-green-500 dark:focus:border-green-500/70"
      > 
        {chain && <img className="w-5 h-5" src={`/icons/${chain?.id}.svg`} />}
        {chain ? <p className="ml-3 mr-auto">{chain?.name} </p> : <p className="ml-auto mr-auto">{'Connect Wallet'}</p>}
      </div>
    </div>
  );
};
