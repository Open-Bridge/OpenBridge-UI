...
contract OpenBridge is OwnerIsCreator {
    ...
    function setProtocol(uint256 _newProtocol) external onlyOwner {
        ...
        returns (bytes32 messageId)
    {
        ...
        uint256 protocolfee = fees + protocol;

        if (protocolfee > s_linkToken.balanceOf(msg.sender))
            revert NotEnoughBalance(s_linkToken.balanceOf(msg.sender), fees);
        //get approval of the user
        //s_linkToken.approve(address(this), fees);
        s_linkToken.transferFrom(msg.sender, address(this), protocolfee);
        //get to transferFrom the user
    ...
    function withdrawFeeToken(address _beneficiary) public onlyOwner {
        // Retrieve the balance of this contract
